...
DESTINATION_DIR = 'splits/casino_response_and_opponent_pref'
...
def get_opponent_context(agent_info: dict):
    agent_pref = agent_info["value2issue"]
    hp_sentence = f'opponent highest priority is {agent_pref["High"]}'
    mp_sentence = f'opponent medium priority is {agent_pref["Medium"]}'
    lp_sentence = f'opponent lowest priority is {agent_pref["Low"]}'
...
def make_write_split_data(data: list, outfile_pth: str):
    ...
    out_file.write('input_seq,response,opponent_pref\n')

    a1 = "mturk_agent_1"
    a2 = "mturk_agent_2"
    for dialogue in data:
        agent_1_context = get_context(dialogue["participant_info"][a1])
        agent_1_opponent_context = get_opponent_context(dialogue["participant_info"][a2])
        agent_2_context = get_context(dialogue["participant_info"][a2])
        agent_2_opponent_context = get_opponent_context(dialogue["participant_info"][a1])
