...
DATA_FILES = 'casino_dual_features' # 'casino_w_all_features' | 'casino' | 'casino_w_task_data' | 'casino_w_outcome' | 'casino_w_all_features'
EPOCHS = 15
...
def preprocess_function(examples):
    ...
    with tokenizer.as_target_tokenizer():
        labels = tokenizer(examples['opt_pref_sentence'], max_length=MAX_OUTPUT_LEN, truncation=True)
...
def compute_metrics(eval_pred):
    ...
    # POTENTIAL REVERT START
    bleu_1_result = []
    bleu_2_result = []
    for lab, pred in zip(decoded_labels, decoded_preds):
        # Syntax: sentence_blue(reference, candidate AKA prediction, weights)
        reference = [lab.lower().split()]
        candidate = pred.lower().split()
        bleu_1_result.append(sentence_bleu(reference, candidate, weights=(1.0, 0.0, 0.0, 0.0)))
        bleu_2_result.append(sentence_bleu(reference, candidate, weights=(0.0, 1.0, 0.0, 0.0)))
    bleu1 = np.mean(bleu_1_result)
    bleu2 = np.mean(bleu_2_result)

    result = {'bleu-1': bleu1, 'bleu-2': bleu2, 'bertscore_precision': np.mean(bertscore_result['precision']), 'bertscore_recall': np.mean(bertscore_result['recall']), 'bertscore_f1': np.mean(bertscore_result['f1'])}
...
trainer.save_model('pretrained_opt_model')
